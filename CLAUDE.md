...
- `bot/ai/tools.py` — OpenAI function-calling tool definitions. Currently has `WebSearch` (DeutschlandGPT `/v2/search`, authenticated with `OPENAI_API_KEY` — always enabled), `WebFetch` (see below, always enabled), and `CreateScheduledEvent` for Discord server event creation. Tools use pydantic models validated via `openai.pydantic_function_tool()`, wrapped in `_tool_definition()` which **pops the `strict: true` key that helper stamps on** — `strict` is OpenAI-only, and a gateway forwarding to a non-OpenAI model translates the tool into that provider's shape and 400s on the extra key (`tools.0.custom.strict: Extra inputs are not permitted`), killing every `/ask` since tools are always sent. `execute_tool_call()` re-validates arguments with the same pydantic model, so nothing is lost. `CreateScheduledEvent` is omitted from `TOOL_DEFINITIONS` when `EVENTS_VOICE_CHANNEL_ID` is unset, so the model never sees a tool it can't fulfill.
